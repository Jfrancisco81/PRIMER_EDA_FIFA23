...
porteros_columnas = ['ID','FullName', 'PhotoUrl', 'Growth', 'TotalStats',
                     'BaseStats','BestPosition','WageEUR', 'ReleaseClause',
                     'ClubPosition','ClubPosition','ClubNumber','ClubJoined',
                     'OnLoad','NationalTeam','NationalPosition','NationalNumber',
                     'PreferredFoot','IntReputation','Positions','AttackingWorkRate',
                     'SkillMoves','DefensiveWorkRate','ShootingTotal','DribblingTotal',
                     'Finishing','HeadingAccuracy','ShortPassing','Volleys','Dribbling',
                     'Curve','FKAccuracy','Strength','Composure','Marking','STRating',
                     'LWRating', 'LFRating', 'CFRating', 'RFRating', 'RWRating','PassingTotal',
                     'CAMRating', 'LMRating', 'CMRating', 'RMRating', 'LWBRating',
                     'CDMRating', 'RWBRating', 'LBRating', 'CBRating', 'RBRating','ContractUntil']

columnas_borradas = ['FullName','PhotoUrl',
                    'Positions','BestPosition',
                    'ClubPosition','ClubJoined',
                    'OnLoad','ReleaseClause',
                    'NationalPosition','NationalNumber',
                    'IntReputation','AttackingWorkRate',
                    'DefensiveWorkRate','GKDiving', 
                    'GKHandling', 'GKKicking', 'GKPositioning',
                    'GKReflexes','STRating', 'LWRating',
                    'LFRating', 'CFRating', 'RFRating', 'RWRating',
                    'CAMRating', 'LMRating', 'CMRating', 'RMRating',
                    'LWBRating','CDMRating', 'RWBRating', 'LBRating', 
                    'CBRating', 'RBRating','GKRating','ClubNumber',
                    'NationalTeam','ContractUntil','WageEUR','WeakFoot',
                    'SkillMoves','SlidingTackle','StandingTackle','ID',
                    'Curve','FKAccuracy','Crossing','ShortPassing',
                    'LongPassing','Growth','Marking','Composure',
                    'TotalStats','BaseStats','Dribbling',]


def eliminar_columnas(df, columnas):
    """
    Esta función recibe un data frame y una lista de columnas a eliminar. 
    """
    return df.drop(columnas, axis=1)

renombrar_columnas = ['Nombre','Edad','altura','peso',
                      'Nacionalidad','General','potencia','Club','Valor','pie_habil',
                      'ritmo','disparo','total_pases','regate',
                      'defensa','fisico','finalización',
                      'precision_cabeza','volea','control_balon',
                      'aceleracion','sprint','agilidad','reaccion',
                      'balance','potencia_disparo','salto','resistencia',
                      'fuerza','tiros_largos','agrecion','intercepciones',
                      'posicionamiento','Vision','Penaltis']


def cambiar_Nombres(df, renombrar_columnas):
    """
    Esta función recibe un data frame y una lista de columnas a renombrarlas
    """
    df.columns = renombrar_columnas
    return df
